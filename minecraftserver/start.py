...
async def proxy():
    try:
        process = await asyncio.create_subprocess_exec(
            "ssh", "-i", "~/.ssh/id_rsa", "-o", "StrictHostKeyChecking=no", "-R", "mineturtle2.nglocalhost.com:25565:localhost:7272", "nglocalhost.com",
    ...
    try:
        ...
        class TheClient(discord.Client):
            async def on_ready(self):
                ...
                if channel:
                    await channel.send(f'<@&1356936657709957150> The server is probably up at [mineturtle2server.nglocalhost.com (port 7272)](https://mineturtle2server.nglocalhost.com)!')
